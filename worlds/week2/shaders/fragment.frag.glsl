...
void main() {
    ...
    Ldir[0] = normalize(vec3(65,65,30));
    Lcol[0] = vec3(.5,.3,.5);

    Ldir[1] = normalize(vec3(-5.,-5.,-3.));
    ...
    Sphere[2]   = vec4(.2,.55,3,.05);
    Ambient[2]  = vec3(.1,0,0);
    Diffuse[2]  = vec3(0.4,.0,.0);
    Specular[2] = vec4(.6,.6,.6,10.); // 4th value is specular power
